...
# ! Renaming Missing Values (NAs)
# df['column_name'] = df['column_name'].replace(to_replace="Missing Value", value=np.nan)
# df['column_name'].replace(to_replace='Missing Value', value=np.nan, inplace=True)

# ! skipna & fillna
# df['Column_Name'].mean(skipna=True)
# df['Column_Name'] = df['Column_Name'].fillna(df['Column_Name'].mean(skipna=True))
...
#! for ranges between n and m
# degrees = np.arange(1, 31)  -> this will give from 1 to 30
